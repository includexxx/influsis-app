# Screen Specs — Creator Profile

| | |
|---|---|
| **Figma node** | [`6001:37822`](https://www.figma.com/design/E7VpnelWNYgzs9WoLLNeh8/Influsis-Project-Brand_App-Version?node-id=6001-37822&m=dev) — "Creator Profile Details - Brand Side_sample 2" |
| **Route** | `/creator/[id]` (`app/(details)/creator/[id].tsx`) |
| **Scene** | `scenes/main/CreatorProfile.tsx` |
| **Data** | `data/creators.ts` |
| **Components used** | `ScreenHeader`, `Image`, `GigCard` (existing); `StarRating`, `ReviewCard` (new, `components/elements/`) |

## Purpose

The full profile view for a single creator — banner photo, avatar, name, bio, active gigs, category tags, and customer reviews. Reached by tapping any creator, anywhere in the app.

## User flow

```
/home (avatar row)              /top-creators (CreatorCard list)
...
`/creator/[id]` is a dynamic route inside the `app/(details)/` route group (outside the `(main)` Tabs group), the same "no tab bar" reasoning as every other screen in that group. Both entry points pass the same `onPress={() => router.push(\`/creator/${item.id}\`)}` — Home's avatar row (now a `Pressable` `CircleAvatar`) and `/top-creators`' `CreatorCard` list — and the "Active Gigs" section's `GigCard`s reuse the exact same `onPress={() => router.push(\`/gig/${gig.id}\`)}` every other gig card in the app already uses.
...
## Data consolidation: one canonical creator list

Before this screen, Home's "Top Rated Creator" row was five plain, identity-less circular headshots (`data/home.ts`'s old `topRatedCreatorAvatars: ImageSourcePropType[]`) with no id linking any of them to the four named creators `/top-creators` already had — fine when nothing needed to look one up individually, but a real problem once tapping an avatar needed to resolve to an actual profile. Consolidated into `data/creators.ts`, now the single source of truth (the same pattern `data/gigs.ts` already established for gigs): `data/home.ts`'s `topRatedCreators` export is now `{id, image}` pairs mapped from the canonical list (down from five avatars to four, matching how many named creators actually exist), and `data/topCreators.ts` re-exports the canonical list wholesale.
...
- **No real backend.** As with every other flow so far, there's no creators API (`docs/PRD.md` §2.2/§4.1) — every creator's `bio`, `categories`, `customerRating`, `activeGigIds` and `reviews` (all new optional fields on the `Creator` type) come from `data/creators.ts`.
- **Figma's one example mirrored across every creator.** Figma shows a single bio, category set (Entertainment/Lifestyle/Sports), rating (4.5), and review (repeated identically for all three review cards, reviewer "Salman Muktadir" included) for its one example profile — applied identically to all four creators rather than inventing distinct profile content Figma doesn't specify, same as Gig Details' "What I will create" breakdown.
...
- **Two new star icon assets, not a reuse of the existing one.** `CreatorCard`'s "Top Rated" badge already has a star icon, but this screen's two star displays are both different shapes (a sharp decorative star and a rounded "material-symbols" star) from that one and from each other — extracted and rasterized separately via `scripts/rasterize-profile-assets.py` rather than force-fit into a visually different existing asset, the same reasoning `CreatorCard`'s own verified-badge icon used.
- **Invalid `id` falls back to Home.** Same as Gig Details — an unmatched `/creator/[id]` renders `<Redirect href="/home" />` rather than a broken page.

## Navigation

- **Entry:** Home's "Top Rated Creator" avatar row (`scenes/main/Home.tsx`), `/top-creators`' `CreatorCard` list (`scenes/main/TopCreators.tsx`).
