...
- **Inert tap targets.** Every card is not wired to an `onPress` handler — there's no campaign/gig/influencer detail screen built yet. Rather than invent fake destinations, these are left as visually-complete but non-functional per Figma, matching this project's established pattern of not building ahead of what's been designed. Every section header's "See all" link, on the other hand, now navigates somewhere — `/notifications`, `/search`, `/live-campaign`, `/brands`, `/campaigns`, `/top-gigs`, and `/top-influencers` respectively (see those screens' specs). Popular Campaigns' "See all" also points at `/campaigns` — there's no separate "popular campaigns" detail screen, and it's the closest existing match.
