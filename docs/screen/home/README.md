...
/home  (default tab on entering the app shell)
  ...
  ├─ tap search bar                → (no search screen yet - inert)
...
| 2 | Search bar | Static (non-editable) search entry point | — |
...
- **Inert tap targets.** The search bar, every "See all" link, and every card are not wired to `onPress` handlers — there's no search screen or campaign/gig detail screen built yet. Rather than invent fake destinations, these are left as visually-complete but non-functional per Figma, matching this project's established pattern of not building ahead of what's been designed. The notification bell is the exception — it now navigates to `/notifications` (docs/screen/notifications/README.md).
