...
/home  (default tab on entering the app shell)
  │
  ├─ tap notification bell        → (no notifications screen yet - inert)
...
- **Inert tap targets.** The notification bell, search bar, every "See all" link, and every card are not wired to `onPress` handlers — there's no notifications screen, search screen, or campaign/gig detail screen built yet. Rather than invent fake destinations, these are left as visually-complete but non-functional per Figma, matching this project's established pattern of not building ahead of what's been designed.
