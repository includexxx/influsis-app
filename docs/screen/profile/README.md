...
- **Logout actually clears session state**, not just a visual confirmation: `removePersistData(DataPersistKeys.USER)`, `setUser(undefined)`, then the `signOut()` thunk (ends the Redux session via `sessionEnded`, resets the `authApi` cache, clears the persisted token pair), then `router.replace('/auth/sign-in')`.
