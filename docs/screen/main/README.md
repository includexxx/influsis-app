...
- A `tabPress` listener in `_layout.tsx` that calls `e.preventDefault()` and pushes `/create` instead of switching tabs. `/create` is the first step of a real 3-step wizard — see [`docs/screen/create-gig/README.md`](../create-gig/README.md) — and, along with its other two steps (`create-gig-pricing`, `create-gig-preview`), is registered in the `(details)` route group (`app/(details)/create.tsx` etc.) rather than here, so the wizard pushes onto the root `Stack` with no tab bar mounted underneath, the same as every other `(details)` screen. `app/(main)/create-gig.tsx` still exists as its own file (Expo Router's file-based Tabs needs one to render the bar button in the right position) but only renders as a `<Redirect href="/home" />` fallback for the edge case of a stale deep link pointing straight at it.

## Scope notes

- **Order is real.** `scenes/main/Order.tsx` has its own Figma design and full spec — see [`docs/screen/orders/README.md`](../orders/README.md) — replacing the bare "not built yet" placeholder that previously stood in for it.
...
- **Home, Message and Profile are real.** `scenes/main/Home.tsx` has its own Figma design and full spec — see [`docs/screen/home/README.md`](../home/README.md). `scenes/main/Message.tsx` likewise — see [`docs/screen/message/README.md`](../message/README.md). `scenes/main/Profile.tsx` is now a settings menu ("Account" in Figma) rather than a read-only field dump — avatar/name/email, then rows into Edit Profile, Security Settings, Billing (inert), My Applications, Help Center, Privacy Policy, and Logout — see [`docs/screen/profile/README.md`](../profile/README.md) for the full flow, including its 5 `(details)`-group sub-screens. None of these have a real backend yet (Home's campaigns/gigs are mock data, Profile's fields write to in-memory Redux state), but all show real structured content rather than a placeholder.
...
- **Within the shell:** tapping Home/Order/Message/Profile switches tabs normally. Tapping Create Gig pushes `/create` (dismissed via its `ScreenHeader` back chevron, `router.back()`) and from there into the rest of the wizard; since the wizard lives in `(details)` rather than `(main)`, each step pushes onto the root `Stack` with the tab bar unmounted underneath, the same as any other `(details)` screen (gig/business/campaign details, chat, notifications, ...) — not the modal slide-up presentation a `presentation: 'modal'` `Stack.Screen` would get, since none of `(details)`'s screens use that option.
