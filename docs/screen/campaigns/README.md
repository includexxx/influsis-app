...
- **No real backend.** As with every other flow so far, there's no campaigns API (`docs/PRD.md` §2.2/§4.1) — content comes from `data/campaigns.ts`'s mock `Campaign[]` array, reusing the existing `Campaign` type and the campaign photos already extracted for Home (`assets/images/home/*.jpg`) rather than exporting duplicates.
