...
- **Three of four rows navigate.** "Mobile banking" carries an `href` to the [Mobile Banking](../mobile-banking/README.md) flow, "Bank transfer" to [Withdraw to Bank](../withdraw-bank/README.md), and "Recent transaction" to the [Transaction](../transactions/README.md) history. "Credit or Debit card" has no destination in the design yet and just takes the pressed state; adding an `href` in `data/balance.ts` is the whole change once one exists.
