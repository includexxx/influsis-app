...
- **Exit:** back chevron (`ScreenHeader`'s `onBack`) → `router.back()` to `/order`, or "Delivery" → `/order/[id]/deliver` (`docs/screen/order-deliver/README.md`). An unknown `id` redirects to `/order` instead of rendering.
