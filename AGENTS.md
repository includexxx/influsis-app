...
Expo SDK 54 / React Native app (iOS, Android, Web from one codebase).

- Dev server: `npm run dev` (also `dev:ios`, `dev:android`, `dev:web`)
- Build (native): `npm run dev:build:mobile` (EAS Build, development profile)
- Build (web): `npm run dev:build:web`
- Lint: `npm run lint`
- Format: `npm run format`
- Test: `npm run test` (Jest + React Native Testing Library, `--passWithNoTests`)

Testing is already set up (Jest/`jest-expo` + RNTL, 70+ component tests under
`components/elements/*/*.test.tsx`). `npm run test` is declared above, so the
Blueprint testing gate is on for logic-bearing steps.

No combined `Verify` command exists yet. `.github/workflows/test.yml` already
runs `format` -> `lint` -> `test` on every PR and push to `dev`/`staging`/`main`.
Run `/ci` or `$ci` if you want a single project-specific `Verify` script wired
into that check.

Browser testing is opt-in and separate from the Expo Web target above. Run
`/browser-tests` or `$browser-tests` to add or normalize a browser harness and
document its exact command as `Browser tests`. Check and Continuous Mode can
then reuse it without installing tooling mid-feature.
